...
## P8 — Production art and presentation

Status: passed

- Timestamp: 2026-07-17T18:33:04+10:00
- Implementation commit: `3efd60f26871de4e3cb34173f751cb38f17230ce`
- Browser seed: solo production `12345`; authoritative two-client regression room `7ASBM5`; manually observed lobby room `63WTJ9`
- Client count: 2 authoritative clients plus the 1,920×1,080 production-browser solo artifact
- Automated checks:
  - `npm test`: all 12 files / 109 tests passed. `npm run build` regenerated byte-stable 2K material atlases, passed TypeScript with zero diagnostics, built the PWA client, and built the authoritative server.
  - `npm run gate:p7:network`: room `7ASBM5` synchronized remote Melder fire, melee, weapon-specific reload, grenade throw/explosion coordinates, and enemy vocal source/species to both real SDK clients.
  - The P8 contract verifies five co-registered 2,048×2,048 PBR atlas channels, production post-processing, original rig targets, all four room identities, a versioned protocol handshake, and the complete menu/HUD/records/game-over presentation surface.
  - The final production artifact is `index-dR9IjPxl.js`, 948.58 kB / 256.75 kB gzip. The generated service worker precaches 12 entries / 4,618.43 KiB.
- Browser acceptance:
  - The exact production artifact loaded seed `12345` with one local player, nine barriers, offline cache `ready`, and no BufferGeometry merge failures. A clean browser session reported zero page or console errors.
  - A deterministic 30-second stress scene retained exactly 24 / 24 live enemies with 66 queued at round 25. The final 1,920×1,080 capture reported 60 FPS, 129 draw calls, and 504k visible triangles—inside the 150-call budget.
  - The full 1,920×1,080 visual tour inspected Start Hall, Armory, Generator, Catwalk, menu, offline state, and stress scene. Evidence is committed under `evidence/p8/`; room-specific lighting, generated PBR materials, structural damage, barriers, machinery, Forge, perk machines, wall buys, viewmodel, fog, grain, and HUD all rendered in the production artifact.
  - Offline mode was exercised with browser networking blocked after cache installation. Reload was served by the service worker, Solo entered active play, the shell displayed `OFFLINE · SOLO ONLY`, and both co-op actions were disabled.
